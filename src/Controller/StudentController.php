...
//    }
	...
	public function input(){
		...
		for ($qNum = $iniQueAfNum ; $qNum < $iniQueAfNum + 10; $qNum++ ) {
			$inputtedLog['answers'][$qNum] = $session->read($this->genSsnAry(['answers',$qNum])) ?: 0;
			$inputtedLog['confidences'][$qNum] = $session->read($this->genSsnAry(['confidences',$qNum])) ?: 2;
		}
		...
		if ($this->request->is('post')) {
			...
			for ($qNum = $iniQueBefNum ; $qNum < $iniQueBefNum + 10; $qNum++ ){
				...
				//解答と自信度をセッションに書き込む
				$session->write($this->genSsnAry(['answers',$qNum]), $answer);
				$session->write($this->genSsnAry(['confidences', $qNum]), $confidence);
			}
		}
	}
	...
	private function genSsnAry(array $children): String{
